...
fzf-project-widget() {
    ...
    local idx=0
    for ((i=1; i < ${#projectFiles[@]}; i++)); do
        if grep '" name:'"${projectName}" ${projectFiles[i]} 2>&1 /dev/null; then
            idx=$i
            break
        fi
    done
    ...
    zle push-line
    BUFFER=" builtin cd -- ${projectDirs[$idx]}; vim -c \"SessionLoad ${projectName}\""
    zle accept-line
    zle reset-prompt
...
bindkey -M emacs '\ep' fzf-project-widget
bindkey -M vicmd '\ep' fzf-project-widget
bindkey -M viins '\ep' fzf-project-widget
